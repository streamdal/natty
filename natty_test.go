...
func GetRandomName(prefix string, retry int) string {
begin:
	name := prefix + left[rand.Intn(len(left))] + "-" + right[rand.Intn(len(right))] //nolint:gosec // G404: Use of weak random number generator (math/rand instead of crypto/rand)
	if name == prefix+"boring_wozniak" /* Steve Wozniak is not boring */ {
		goto begin
	}
